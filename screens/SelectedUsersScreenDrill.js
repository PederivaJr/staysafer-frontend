...
import OptionMenuEvacListDrill from "../components/OptionMenuEvacListDrill";
...
function SelectedUsersScreenDrill({ route }) {
  ...
  if (!list)
    list = authContext.evacLists.find((list) => list.type.includes("drill"));
  ...
  useEffect(() => {
    if (isFocused && list?.list_id) {
      console.log(
        "SelectedUsersScreenDrill focused, loading evac list users from MySQL API (source of truth)"
      );
  ...
  useEffect(() => {
    if (lastUpdate && authContext.selectedUsersDrill?.length >= 0) {
      if (lastUpdate.action === "contact_added") {
        console.log(
          "Applying real-time contact addition to existing drill lists"
        );
        // For additions, refresh the complete list to get the new contact
        refreshEvacListUsers();
      } else if (lastUpdate.action === "contact_removed") {
        console.log(
          "Applying real-time contact removal to existing drill lists"
        );
  ...
  useEffect(() => {
    if (realtimeSelectedUsers && realtimeSelectedUsers.length >= 0) {
      console.log(
        "SelectedUsersScreenDrill: Updating selected users from real-time data"
      );
  ...
  useEffect(() => {
    if (realtimeSelectedUsersDrill && realtimeSelectedUsersDrill.length >= 0) {
      console.log(
        "SelectedUsersScreenDrill: Updating drill users from real-time data"
      );
  ...
  useEffect(() => {
    if (realtimeTempContacts && realtimeTempContacts.length >= 0) {
      console.log(
        "SelectedUsersScreenDrill: Updating temp contacts from real-time data"
      );
  ...
  useEffect(() => {
    if (realtimeCompanyUsers && realtimeCompanyUsers.length >= 0) {
      console.log(
        "SelectedUsersScreenDrill: Updating company users from real-time data"
      );
  ...
  useEffect(() => {
    if (lastUpdate) {
      console.log("SelectedUsersScreenDrill: Update received:", {
  ...
  const refreshEvacListUsers = async () => {
    ...
    try {
      ...
      console.log(
        "Manual refresh evac list users (drill):",
      ...
      console.error("Manual refresh evac list users (drill) failed:", error);
    }
  };
  ...
  const clearList = async () => {
    ...
    console.log("clear evac list API: ", JSON.stringify(result, null, 2));
  ...
  const handleAgree = async () => {
    // Trigger the location permissions request.
  ...
  const handleSkip = () => {
    setDisclosureModalVisible(false); // Close the modal
  };
      ...
      <View style={styles.addContactContainer}>
        <View style={styles.evacLeft}>
          <Text style={styles.infinity}>
            {t("selected")}
            {": "}
            {authContext.selectedUsersDrill?.length
              ? authContext.selectedUsersDrill.length
              : 0}
            {usersLoading &&
        ...
        <OptionMenuEvacListDrill onSelectAction={handleAction} />
      </View>
...
export default SelectedUsersScreenDrill;
