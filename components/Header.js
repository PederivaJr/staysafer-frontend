...
function Header({ icon, navigation, ...otherProps }) {
  ...
  useEffect(() => {
    if (evacuation !== null) {
      console.log(
        "Header: Updating evacuation from real-time data:",
        evacuation
      );
      ...
      console.log("Header: Clearing evacuation - no active evacuation found");
  ...
  useEffect(() => {
    if (error) {
      console.warn("Header: Evacuation status error:", error);
    }
  }, [error]);
    ...
    >
      ...
      <View style={styles.headerIcons}>
        ...
        <View style={styles.alarmStatus}>
          ...
          {/* Show a subtle loading indicator when evacuation status is loading */}
          {loading && !authContext.evacuation && (
            <MaterialCommunityIcons
              size={20}
